...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${baseURL}/backend/clientadd.php`, {
        method: "POST",
        body: form,
      });
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-label">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 ml-10 pr-10 mb-0">
                <div className="flex items-center mb-2 mr-4">
                  ...
                  <button
                    type="submit"
                    className="hover:bg-prime border-2 border-prime font-bold text-sm text-prime hover:text-white py-2 px-4 rounded-md shadow focus:outline-none transition ml-4"
                  >
                    Submit
                  </button>
                </div>
            ...
            <TableBody>
              {currentTickets.map((user, i) => (
                <TableRow key={user.id} className="hover:bg-gray-100">
                  ...
                  <TableCell align="center" style={{ padding: '10px' }} className="border-t">{i + offset}</TableCell>
                  <TableCell align="center" style={{ padding: '10px' }} className="border-t">{user.name}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
